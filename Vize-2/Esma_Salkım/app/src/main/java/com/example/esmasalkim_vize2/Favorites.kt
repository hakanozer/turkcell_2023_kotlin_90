...
class Favorites : AppCompatActivity() {
    ...
    private lateinit var favoriteProductsRecyclerView: RecyclerView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Favori ürünleri gösteren RecyclerView
        favoriteProductsRecyclerView = findViewById(R.id.recyclerView1)
        favoriteProductsRecyclerView.layoutManager = LinearLayoutManager(this)
        val favoriteProductsAdapter = ProductAdapter(allProducts)
        favoriteProductsRecyclerView.adapter = favoriteProductsAdapter
        ...
        dummyService = ApiClient.getClient()
            .create(DummyService::class.java) // interface class haline gelmiş oldu. Buna reflection denir.
        ...
        dummyService.login(userSend).enqueue(object : Callback<JWTUser> {
            ...
            override fun onResponse(call: Call<JWTUser>, response: Response<JWTUser>) {
                ...
                Log.d(
                    "Status",
                    status.toString()
                ) // username ve password yanlış yazılırsa lod'da Status değeri 400 döner. Doğru yazılmışsa 200 döner.
                ...
                if (status == 200) {   // bu kontrolun yapılması şart
                    ...
                    if (user != null) {  // bunun altında istediğimiz gibi gezintiye çıkabiliriz
                        Log.d(
                            "User",
                            user.toString()
                        ) // toString ile içindeki bütün verilere ulaşabiliriz
    ...
    private fun parseProducts(json: JSONObject): List<Product> {
        ...
        for (i in 0 until productsArray.length()) {
            ...
            if (allLikeProduct.contains(id.toInt())) {
                val product = Product(
                    id,
                    title,
                    description,
